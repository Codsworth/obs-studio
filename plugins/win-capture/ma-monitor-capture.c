...
#define TEXT_MONITOR_CAPTURE obs_module_text("MultiAdapterMonitorCapture")
		...
		obs_data_t *settings)
{
	...
	dc_capture_init(&capture->data, monitor.rect.left, monitor.rect.top,
			width, height, capture->capture_cursor, true);
}
...
static void check_and_resize_capture(struct monitor_capture *capture)
{
	struct monitor_info monitor = { 0 };
	uint32_t width, height;

	monitor.desired_id = capture->monitor;
	EnumDisplayMonitors(NULL, NULL, enum_monitor, (LPARAM)&monitor);

	width = monitor.rect.right - monitor.rect.left;
	height = monitor.rect.bottom - monitor.rect.top;

	if (width != capture->data.width || height != capture->data.height)
	{
		dc_capture_free(&capture->data);
		dc_capture_init(&capture->data, monitor.rect.left, monitor.rect.top,
...
static void monitor_capture_tick(void *data, float seconds)
{
	...
	check_and_resize_capture(capture);
...
struct obs_source_info ma_monitor_capture_info = {
	.id             = "ma_monitor_capture",
